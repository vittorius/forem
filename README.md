# Effects playground (based on [forem](https://github.com/forem/forem))

This is an existing RoR application I know nothing about. We'll be adding some effects without much ado.

## Be prepared

- Install ruby 2.7.2. 
- Make sure `bundle install` works. 
- Have PG 11+.
- Have redis.
- Create database with `bundle exec rake db:create`.
- Check if `rspec spec/initializers/rack/attack_spec.rb` passes.
- If something doesn't work, try installing yarn + `yarn install`.
- These instructions can be updated later.
